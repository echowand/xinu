	...
	)
{
	...
	int32	tcplen;			/* length of TCP segment	*/
	...
	/* Purdue localism: to avoid problems with rogue Linksys	*/
	/*	routers, discard packets on lab network sent to or	*/
	/*	from 192.168.0.0/16					*/
	...
	switch (pktptr->net_ipproto) {
	    ...
	    case IP_TCP:
		tcplen = pktptr->net_iplen - IP_HDR_LEN;
		if (tcp_cksum((char *)&pktptr->net_tcpsport, tcplen) != 0){
			freebuf((char *)pktptr);
			return;
		}
		tcp_ntoh(pktptr);
	...
	/* If interface does not yet have a valid address, accept the	*/
	/*	packet (to handle DHCP replies)				*/
	...
	/* If the address on an incoming packet matches the unicast	*/
	/* or brodcast address of the interface, send the packet to	*/
	/* the local stack for processing.				*/
	...
	/* Drop the packet because it's not for us			*/
